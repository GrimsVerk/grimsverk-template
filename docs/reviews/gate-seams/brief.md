# Brief — adversarial review of the permission and verification machinery

The instruction given to the reviewing agent, verbatim. Recorded alongside
`FINDINGS.md` because a review is only re-runnable if the question it was asked
is on the record: a later reader needs to know what was in scope, what was
declared out of scope, and which postures the reviewer was told to separate.

- **Target:** `grimsverk-template`, branch `claude/copier-template-automation-rii78u`
- **Mode:** read-only — no code written, no files changed, no pull request opened
- **Companion:** a separate review covers document *shape*; this one covers machinery

**This is Prompt A.** The text below is byte-identical to
[`../adversarial-review/prompt-a-gate-machinery.md`](../adversarial-review/prompt-a-gate-machinery.md),
which is the authoritative copy and landed on this branch while the review was
running. It is reproduced here so this directory is a complete record on its
own — the same reason `adversarial-review/README.md` inlines both prompts. If
the two ever disagree, the file in `adversarial-review/` is the original and
this one is the stale copy.
